...
<ul class="hkb-archive hkb-archive--<?php echo $columns; ?>-cols clearfix">
    ...
    <li>
        ...
        <div class="hkb-category__header">
            <h2 class="hkb-category__title"><a href="<?php echo esc_attr(get_term_link($tax_term, 'ht_kb_category')) ?>" title="<?php echo sprintf( __( 'Показать все записи в %s', 'helpguru' ), $tax_term->name ) ?>"><?php echo $tax_term->name ?></a>
                <?php if ( hkb_archive_display_subcategory_count() ) : ?><span class="hkb-category__count"><?php echo sprintf( _n( '1 Запись', '%s записей', $tax_term->count, 'helpguru' ), $tax_term->count ); ?></span><?php endif; ?>
            </h2>
        ...
        <?php if( !empty( $cat_posts ) && !is_a( $cat_posts, 'WP_Error' ) ): ?>
            ...
            <a class="hkb-category__view-all" href="<?php hkb_term_link($tax_term); ?>"><?php _e( 'Показать все', 'helpguru' ); ?></a>
        <?php endif; ?>
