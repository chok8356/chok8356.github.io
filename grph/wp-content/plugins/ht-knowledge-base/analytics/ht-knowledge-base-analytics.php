...
if( !class_exists( 'HKB_Analytics_Module' )) {
  ...
  if(!defined('HT_KB_STORE_URL')){
    define('HT_KB_STORE_URL', 'http://herothemes.com/plugins/heroic-wordpress-knowledge-base/');
  }
  ...
  class HKB_Analytics_Module {
    ...
    function ht_hkba_register_menu_page() {
        ...
        if(apply_filters('hkb_analytics_supported', true)){
            ...
            add_submenu_page( 'edit.php?post_type=ht_kb', __('Heroic Knowledge Base Analytics', 'ht-knowledge-base'), __('Analytics', 'ht-knowledge-base'), 'manage_options', 'hkb-analytics', array($this, 'ht_hkba_page_callback'));
        }
    }
    ...
    function hkba_admin_scripts() {
        ...
        if(isset($_GET['page']) && 'hkb-analytics' ===  $_GET['page']) {
            ...
            wp_enqueue_script( 'ht-analytics', plugins_url('/js/ht-analytics.js',__FILE__), array('jquery', 'data-tables-script', 'jquery-ui-core', 'jquery-ui-datepicker', 'backbone', 'underscore'), '1.0.0', true );
